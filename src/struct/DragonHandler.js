...
const { DragonHandlerEvents } = require("../util/Constants");
 ...
 * @param {DragonClient} client - The Dragon client.
 * @param {DragonHandlerOptions} options - Options for module loading and handling.
 * @extends {EventEmitter}
 */
class DragonHandler extends EventEmitter {
  constructor(client, {
    directory,
    classToHandle = DragonModule,
    ...
    /**
         * Modules loaded, mapped by ID to DragonModule.
         * @type {Collection<string, DragonModule>}
         */
  ...
  /**
     * Registers a module.
     * @param {DragonModule} mod - Module to use.
  ...
  load(thing, isReload = false) {
    ...
    if (this.modules.has(mod.id)) throw new DragonError("ALREADY_LOADED", this.classToHandle.name, mod.id);

    this.register(mod, isClass ? null : thing);
    this.emit(DragonHandlerEvents.LOAD, mod, isReload);
    return mod;
  }
  ...
  remove(id) {
    ...
    if (!mod) throw new DragonError("MODULE_NOT_FOUND", this.classToHandle.name, id);

    this.deregister(mod);

    this.emit(DragonHandlerEvents.REMOVE, mod);
    return mod;
  }
  ...
  reload(id) {
    const mod = this.modules.get(id.toString());
    if (!mod) throw new DragonError("MODULE_NOT_FOUND", this.classToHandle.name, id);
    if (!mod.filepath) throw new DragonError("NOT_RELOADABLE", this.classToHandle.name, id);
...
module.exports = DragonHandler;

/**
 * Emitted when a module is loaded.
 * @event DragonHandler#load
 * @param {DragonModule} mod - Module loaded.
 * @param {boolean} isReload - Whether or not this was a reload.
 */

/**
 * Emitted when a module is removed.
 * @event DragonHandler#remove
 * @param {DragonModule} mod - Module removed.
 */

/**
 * Options for module loading and handling.
 * @typedef {Object} DragonHandlerOptions
 * @prop {string} [directory] - Directory to modules.
 * @prop {Function} [classToHandle=DragonModule] - Only classes that extends this class can be handled.
