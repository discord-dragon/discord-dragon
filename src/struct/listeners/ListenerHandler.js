...
const DragonHandler = require("../DragonHandler");
 ...
 * @param {DragonHandlerOptions} options - Options.
 * @extends {DragonHandler}
 */
class ListenerHandler extends DragonHandler {
  constructor(
    client,
    {
      directory,
  ...
  addToEmitter(id) {
    ...
    if (!listener) {
      throw new DragonError("MODULE_NOT_FOUND", this.classToHandle.name, id);
    }
    ...
    if (!isEventEmitter(emitter)) {
      throw new DragonError("INVALID_TYPE", "emitter", "EventEmitter", true);
    }
  ...
  setEmitters(emitters) {
    for (const [key, value] of Object.entries(emitters)) {
      if (!isEventEmitter(value)) {
        throw new DragonError("INVALID_TYPE", key, "EventEmitter", true);
      }
